...
def do_and_form(expressions, env):
    ...
    if expressions.second is nil:
        curr_expr = scheme_eval(expressions.first,env, True)
    else:
        curr_expr = scheme_eval(expressions.first,env)

    if not scheme_truep(curr_expr):
        return curr_expr
    elif scheme_truep(curr_expr) and expressions.second == nil:
        return curr_expr
    elif scheme_truep(curr_expr):
...
def do_or_form(expressions, env):
    ...
    if scheme_truep(curr_expr):
        return curr_expr
    elif not scheme_truep(curr_expr) and expressions.second is nil:
        return curr_expr
    elif not scheme_truep(curr_expr):
...
def make_let_frame(bindings, env):
    ...
    while bindings != nil:
        ...
        a = Pair(binds.first, a)
        b = Pair(scheme_eval(binds.second.first, env), b)
...
def optimize_tail_calls(original_scheme_eval):
    ...
    def optimized_eval(expr, env, tail=False):
        ...
        # BEGIN
        if not tail:
            return scheme_eval(expr, env, False)
        # END
